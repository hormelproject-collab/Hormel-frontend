...
export default function ModifyExistingBOMData() {
  ...
  const getPriorityFieldError = (value) => {
    ...
    if (trimmed === "") return "";
    const num = Number(trimmed);
    if (!Number.isFinite(num) || !Number.isInteger(num)) return "Priority must be a whole number.";
    return "";
  };

  const hasMissingStandardUsage = componentItems.some((item) => toText(item.standard_usage) === "");
  const hasMissingCoProductFields = producedCoProduct ? coProducts.some((cp) => toText(cp.item) === "" || toText(cp.qty) === "" || toText(cp.resource) === "" || toText(cp.itemBomRoutingPriority) === "") : false;
  ...
  const hasInvalidCoProductPriority = producedCoProduct ? coProducts.some((cp) => !!getPriorityFieldError(cp.itemBomRoutingPriority)) : false;
  const hasInvalidNumericValues = hasInvalidComponentUsage || hasInvalidCoProductQty || hasInvalidCoProductPriority;
  ...
  const validateAndNavigate = () => {
    ...
    if (hasMissingCoProductFields) return setValidationError("Please fill in Item, Resource, Item BOM Routing Priority and Qty Produced for all co-product rows before moving forward.");
    if (hasInvalidNumericValues) return setValidationError("Standard Usage must be greater than 0. Qty Produced must be greater than 0 and less than 1. Item BOM Routing Priority must be a whole number.");
    ...
    const removedCoProducts = initialCoProducts.filter((orig) => { const key = toText(orig.original_item || orig.item); if (!key) return false; return !coProducts.some((cp) => toText(cp.original_item || cp.item) === key); }).map((item) => ({ ...item, removed: true }));
    setValidationError("");
        ...
        {detailsLoading ? (
          ...
          <div style={{ ...styles.card, marginTop: 14 }}>
            ...
            {coProducts.length === 0 ? <div style={styles.emptyBox}>No co-product items. Click "Add Co-Product" to add one.</div> : (
              <div style={styles.editTableWrap}>
                <div style={styles.coProductEditTableHeader}><div>Co-Product Item</div><div>Item Description</div><div>Resource</div><div>Item BOM Routing Priority</div><div>Qty Produced Per</div><div /></div>
                {coProducts.map((cp, index) => (
                  <div key={cp.id || cp.original_item || cp.item || index} style={styles.coProductEditTableRow}>
                    ...
                    <div style={styles.inputWithError}><input type="number" min="0" step="1" value={cp.itemBomRoutingPriority || ""} onChange={(e) => updateCoProduct(index, "itemBomRoutingPriority", e.target.value)} style={getPriorityFieldError(cp.itemBomRoutingPriority) ? styles.tableInputError : styles.tableInput} placeholder="Priority" />{getPriorityFieldError(cp.itemBomRoutingPriority) ? <div style={styles.inlineError}>{getPriorityFieldError(cp.itemBomRoutingPriority)}</div> : null}</div>
